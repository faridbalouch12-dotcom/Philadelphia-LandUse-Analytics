# Zoning Year-to-Year Comparability Plan (Draft)
...
- Mapping strategy for the `dim_zoning_code` lookup table and `stg_zoning_code_crosswalk`
...
This produces a **flagged code diff** per vintage transition. The `dim_zoning_code` lookup table — which contains the union of all `long_code` values across all 5 vintages — serves as the master inventory for this comparison.
...
### 2a. `dim_zoning_code` — Lookup Table (Warehouse)
...
- Lookup vs SCD2 explainer: [`notes/kimball/lookup_vs_scd2_explainer.html`](../notes/kimball/lookup_vs_scd2_explainer.html)
...
| Date       | Change                        | Author |
|------------|-------------------------------|--------|
| 2026-03-08 | Initial draft created         | Farid  |